...
import { ZOOM_STOPS } from "./pageZoom";
...
export function ControllerApp() {
  ...
  const busy = controller.isPlaying || controller.isExporting;

  /** The next zoom stop in `direction`, or the current one at either end. */
  const stepZoom = (direction: -1 | 1) => {
    const at = ZOOM_STOPS.indexOf(controller.pageZoom);
    const next = (at === -1 ? ZOOM_STOPS.indexOf(100) : at) + direction;
    ...
    <div class="dolly-app">
      <div class="dolly-app-header">
        <div class="dolly-app-header__left">
          ...
          {/* Page zoom sits with the frame controls: it scales the recorded
              page's content, and leaves the frame itself alone. */}
          <div class="dolly-zoom" title="Zoom the page">
            <button
              class="dolly-zoom__step"
              onClick={() => void controller.setPageZoom(stepZoom(-1))}
              disabled={busy || controller.pageZoom === ZOOM_STOPS[0]}
              aria-label="Zoom out"
            >
              <ZoomOutIcon size={15} />
            </button>
            <span class="dolly-zoom__read">{controller.pageZoom}%</span>
            <button
              class="dolly-zoom__step"
              onClick={() => void controller.setPageZoom(stepZoom(1))}
              disabled={
                busy ||
                controller.pageZoom === ZOOM_STOPS[ZOOM_STOPS.length - 1]
              }
              aria-label="Zoom in"
            >
              <ZoomInIcon size={15} />
        ...
        >
          {controller.isExporting ? (
            ...
            >
              ...
              <ChevronDownIcon
                size={15}
                class="dolly-app-header__export-chev"
              />
            </button>
          )}
